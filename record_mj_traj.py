...
VIDEO_PATH = BASE_DIR / "data" / "traj_front_camera.mp4"
TRACKS_PATH = BASE_DIR / "data" / "traj_rope_tracks_txy.npy"
OCCLUSION_PATH = BASE_DIR / "data" / "traj_rope_tracks_occluded.npy"
...
camera_id = mj.mj_name2id(model, mj.mjtObj.mjOBJ_CAMERA, "front_camera")
...
# def generate_piecewise_trajectory(start, end, n_steps):
    ...
    while data.time < DURATION:
        ...
        if len(frames) < int(data.time * FPS):
            ...
            for i, geom_id in enumerate(tracked_geom_ids):
                ...
                rope_pos_homogenous = np.ones((4, 1))
                rope_pos_homogenous[:3] = rope_pos

                # project 3d to 2d
                xs, ys, s = camera_matrix @ rope_pos_homogenous
                ...
                if selected_geom_id != geom_id:
                    # occluded
                    occluded[len(frames) - 1, i] = 1

                tracks_txy[len(frames) - 1, i, :] = np.array([frame_num, x, y])
